...
let results = []; // Array to store the converted date results
...
app.post('/convert', upload.single('file'), async (req, res) => {
  ...
  try {
    fs.createReadStream(file.path)
        ...
        if (date) {
          try {
            ...
            if (convertedDate) {
              ...
              results.push(rowData); // Push rowData to the results array
            }
          } catch (error) {
      ...
      .on('end', async () => {
        // After parsing is done, format the results array and write to CSV
        const formattedData = formatApiResponseData(results);
        ...
        // Write the CSV file
        fs.writeFile(outputPath, formattedData, (err) => {
          if (err) {
